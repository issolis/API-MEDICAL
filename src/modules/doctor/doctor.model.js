...
export default class Doctor {
    ...
    static async getByUserId(userId, client = pool) {
        return await Surgery.getByUserId(
            userId,
            { excludeRole: "patient" },
            client
        );
    ...
    static async getSurgeryById(userId, surgeryId, client = pool) {
        const surgery = await Surgery.getById(
            surgeryId,
            { excludeRole: "patient" },
            client
        );
        ...
        const surgeries = await Surgery.getByUserId(
            userId,
            { excludeRole: "patient" },
            client
        );

        const belongsToDoctor = surgeries.some(s => s.id === surgeryId);
